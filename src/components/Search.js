function Search({handleSearchSubmit,query,completeQuery,handleQueryUpdate,handleBackToOrigin}){
  return(
    <form onSubmit={handleSearchSubmit} className="search-group">
      <input type="text" className="search-box" value={query} onChange={handleQueryUpdate} placeholder="Search here" />
      ...
      {
        completeQuery!==""?<button type="button" className="go-back" onClick={handleBackToOrigin}>Go Back</button>:""
      }
    </form>
  )
}
